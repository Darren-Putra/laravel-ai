        ...
        <div class="flex-1 overflow-y-auto py-4 space-y-4 custom-scrollbar" id="chat-window">
            <template x-for="msg in messages">
                <div :class="msg.role === 'user' ? 'flex justify-end' : 'flex justify-start'">
                    <div :class="msg.role === 'user' ? 'bg-blue-600' : 'bg-gray-800 border border-gray-700'"
                        class="max-w-[80%] rounded-lg p-3 shadow-sm">
                        <p class="text-sm font-semibold mb-1" x-text="msg.role === 'user' ? 'Anda' : 'AI Specialist'">
                        </p>
            ...
            <div x-show="loading" class="flex justify-start">
                <div class="bg-gray-800 border border-gray-700 rounded-lg p-3 animate-pulse">
                    Thinking...
                </div>
            </div>
        </div>
        ...
        <div class="py-4 border-t border-gray-700">
            <form @submit.prevent="sendMessage" class="flex gap-2">
                <input type="text" x-model="userInput"
                    ...
                    placeholder="Tanya sesuatu tentang Laravel...">
                <button type="submit" :disabled="loading"
                    class="bg-blue-600 hover:bg-blue-700 px-6 py-2 rounded-lg font-semibold disabled:opacity-50">
                    Kirim
                </button>
            </form>
        </div>
                ...
                async sendMessage() {
                    ...
                    try {
                        const response = await fetch(
                            `/tanya-ai?q=${encodeURIComponent(userMsg)}`
                        );
